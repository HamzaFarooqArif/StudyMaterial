...
def key_of_min_value(d):
    ...
    'c'
    """
    # BEGIN Question 0                  #The key function is called with each element of the list, and the return values are used for comparison
    return min(d, key = lambda x: d[x]) # When we call key function with specific input it give specific value then the input is returned on which it gives least value.
    # END Question 0
...
def enumerate(s, start=0):
    ...
    """
    # BEGIN Question 0
    return zip([x for x in range(start, start + len(list(s)))], s)
    # END Question 0
