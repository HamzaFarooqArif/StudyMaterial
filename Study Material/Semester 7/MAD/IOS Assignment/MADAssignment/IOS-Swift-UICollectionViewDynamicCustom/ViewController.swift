...
//        collectionView.backgroundColor = UIColor.green
    ...
    {
        let today = Date()
        let modifiedDate = Calendar.current.date(byAdding: .day, value: offset, to: today)!
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.labelNowDate.text = getDate(offset: 0) //desF[cellIndex]
        cell.labelAfterDate.text = getDate(offset: 10)//titlesF[cellIndex]
        //cell.labelDetails.font = cell.labelDetails.font.withSize(closeFrameSize * relativeFontCellDescription)
        cell.labelName.text = titlesF[cellIndex]
        
        cell.new_button.tag = indexPath.row
        cell.new_button.addTarget(self, action: #selector(btnPress(sender:)), for: .touchUpInside)
   ...
   @objc func btnPress(sender: UIButton){
    ...
    print(sender.tag)
       ...
       vc.name = titlesF[sender.tag]
       vc.image = imagesF[sender.tag]
       
       self.navigationController?.pushViewController(vc, animated: true)
   }
    
    @IBAction func btn_new_pressed(_ sender:
        UIButton) {
//        print(sender.tag)
//        let vc = storyboard?.instantiateViewController(withIdentifier: "DetailsViewController") as! DetailsViewController
//
...
extension ViewController : UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        let heightVal: CGFloat = self.view.frame.height
        let widthVal: CGFloat = self.view.frame.width
